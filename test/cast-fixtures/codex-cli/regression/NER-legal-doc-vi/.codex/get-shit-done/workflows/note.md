...
- **Global scope**: `E:/NER/.codex/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `E:/NER/.codex/notes/`)
...
2. Glob `E:/NER/.codex/notes/*.md` (if directory exists) — global notes
...
2. **No `.planning/`**: Falls back to global `E:/NER/.codex/notes/` — works in any directory
