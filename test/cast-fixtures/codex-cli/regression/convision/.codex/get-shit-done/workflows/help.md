...
- Saves timestamped note to `.planning/notes/` (or `/Users/wilderherrera/Desktop/convision/.codex/notes/` globally)
...
**`/gsd-qa-explore [rol]`** (skill: `gsd-qa-explore`)
Exploración QA funcional **con navegador** sobre la app Convision (sin fase GSD obligatoria).

- Login por rol, recorrido del menú lateral y rutas del mapa (`docs/QA_MAPA_EXPLORACION.md`)
- Requiere front `http://localhost:4200`, API `http://localhost:8000` y MCP de navegador en la sesión
- Escribe hallazgos en `.planning/qa/FINDINGS-*.md` para handoff al agente con regla `convision-qa-fixer`

**No es** reemplazo de `/gsd-verify-work` (UAT conversacional por fase).

Usage: `/gsd-qa-explore`
Usage: `/gsd-qa-explore admin`
Usage: `/gsd-qa-explore receptionist solo COMERCIAL`
